(ns subtide.linter
  "Helpers for linting subtide vars correctly (using clj-kondo)."
  (:require
...
(defn- linter-hooks
  ...
  [vars]
  ['(ns subtide.subtide.clj-kondo-hooks
      ...
      (let [[var-sym docstring binding-vec & body] (rest (:children node))
            new-node (api/list-node
                      (list
                       ...
                       (api/list-node
                        (list*
                         (api/token-node 'subtide.helpers.lib/defunk)
                         var-sym
                         docstring
                         binding-vec
                         body))
...
(defsynth ppp2
  ...
  (out bus (* amp (mda-piano :freq (midicps 50)
                                         ...
                                         [(symbol v)
                                          (merge (select-keys var-meta [:arglists])
                                                 ...
                                                 (when (or (:arglists-modified? var-meta)
                                                           (contains? #{:subtide.sc.machinery.ugen.fn-gen/ugen
                                                                        :subtide.sc.defcgen/cgen}
                                                                      (:type var-value))
                                                           (contains? #{:subtide.helpers.lib/unk}
                                                                      (:type var-meta))
                                                           (instance? subtide.sc.synth.Synth var-value)
                                                           (instance? subtide.studio.inst.Inst var-value))
                                                   {:arglists '([& _])
                                                    :special? true}))])))
                               (into {}))))
   ...
   '(defn immigrate-subtide-api [_]
      (intern-vars vars))

   '(defn intern-ugens [_]
      (intern-vars (->> vars
                        (filter (comp #{"subtide.sc.ugens"} namespace first))
                        (into {}))))

   '(comment

      (->> (-> (str (:node (immigrate-subtide-api nil)))
...
(defn emit!
  ...
  (let [hooks (linter-hooks
               (->> (find-vars {:namespaces subtide.api/immigrated-namespaces})
                    (remove (comp :private meta))
                    #_(take 10)))
        config '{:lint-as
                 {subtide.music.pitch/defratio
                  clojure.core/def}

                 :hooks
                 {:analyze-call
                  {subtide.api/immigrate-subtide-api
                   subtide.subtide.clj-kondo-hooks/immigrate-subtide-api

                   subtide.sc.synth/demo
                   subtide.subtide.clj-kondo-hooks/demo
                   ...
                   subtide.core/defsynth
                   subtide.subtide.clj-kondo-hooks/defsynth

                   subtide.live/defsynth
                   subtide.subtide.clj-kondo-hooks/defsynth

                   subtide.sc.synth/defsynth
                   subtide.subtide.clj-kondo-hooks/defsynth

                   subtide.core/definst
                   subtide.subtide.clj-kondo-hooks/defsynth

                   subtide.live/definst
                   subtide.subtide.clj-kondo-hooks/defsynth

                   subtide.studio.inst/definst
                   subtide.subtide.clj-kondo-hooks/defsynth

                   subtide.core/defsynth-load
                   subtide.subtide.clj-kondo-hooks/defsynth-load

                   subtide.live/defsynth-load
                   subtide.subtide.clj-kondo-hooks/defsynth-load

                   subtide.sc.synth/defsynth-load
                   subtide.subtide.clj-kondo-hooks/defsynth-load

                   subtide.sc.machinery.ugen.fn-gen/intern-ugens
                   subtide.subtide.clj-kondo-hooks/intern-ugens

                   subtide.synth.stringed/gen-stringed-synth
                   subtide.subtide.clj-kondo-hooks/gen-stringed-synth

                   subtide.sc.defcgen/defcgen
                   subtide.subtide.clj-kondo-hooks/defcgen

                   subtide.core/defcgen
                   subtide.subtide.clj-kondo-hooks/defcgen

                   subtide.live/defcgen
                   subtide.subtide.clj-kondo-hooks/defcgen

                   subtide.helpers.lib/defrecord-ifn
                   subtide.subtide.clj-kondo-hooks/defrecord-ifn

                   subtide.sc.machinery.ugen.check/defcheck
                   subtide.subtide.clj-kondo-hooks/defcheck

                   subtide.byte-spec/defspec
                   subtide.subtide.clj-kondo-hooks/defspec

                   clj-native.direct/defclib
                   subtide.subtide.clj-kondo-hooks/defclib

                   subtide.sc.machinery.defexample/defexamples
                   subtide.subtide.clj-kondo-hooks/defexamples

                   subtide.helpers.lib/defunk
                   subtide.subtide.clj-kondo-hooks/defunk

                   subtide.sc.envelope/defunk-env
                   subtide.subtide.clj-kondo-hooks/defunk-env}}
                 ...
                 {subtide.studio.util
                  ...
                  subtide.sc.ugens
                  {:linters {:inline-def {:level :off}}}}}
        cfg-file (io/file ".clj-kondo" "subtide" "do_not_commit_me" "config.edn")
        hooks-file (io/file ".clj-kondo" "subtide" "do_not_commit_me" "subtide" "subtide" "clj_kondo_hooks.clj")]

...
(defmacro at-offset
  "Schedule server communication - specify that communication messages
   ...
   See `subtide.sc.server/at` for more details."
  [offset-ms & body]
...
(defn sc-debug-on
  "Turn on output from both the Subtide and the audio server."
  []
  (log/set-level! :debug)
  (sc-osc-debug-on)
  (snd "/dumpOSC" 1))
...
(defn sc-debug-off
  "Turn off debug output from both the Subtide and the audio server."
  []
  (log/set-level! :error)
  (sc-osc-debug-off)
  (snd "/dumpOSC" 0))

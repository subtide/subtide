(ns subtide.examples.buses.ambisonics
  (:use subtide.live))
...
;; 2D Ambisonic audio pipeline in Subtide.

;; Subtide's default definst assumes a stereo output pipeline.
...
;; the plane may be as dynamic as anything else in Subtide.
...
(defn create-ambisonic-mixer [{:keys [n-chans] :as _inst} & params]
  (cond
    ...
    ;; Whereas the default output mixers of Subtide instruments ignore
...
(defn- def-foa-output-synth [n-speakers orientation]
  ...
  ;; >> Argument for ugen DecodeB2 must be a number, yet found: #subtide.sc.machinery.ugen.sc_ugen.ControlProxy
  (defsynth foa-output
...
(defn stop-foa
  "Restore the default Subtide output pipeline."
  []
...
(comment
  ...
  (require '[subtide.inst.drum :as drum])
  (drum/bing)

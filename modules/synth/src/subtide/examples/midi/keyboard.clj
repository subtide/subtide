(ns subtide.examples.midi.keyboard
  (:use subtide.live))
...
; subtide.studio.midi now includes the beginnings of a higher level midi interface
; that helps improve on this.  By default Subtide will detect and listen to all
; midi input devices.  These midi messages are then sent as events, which can be received
; with the Subtide event system.
...
; envelope so the release starts.  This is what subtide.studio.midi/midi-poly-player

(ns subtide.sc.machinery.ugen.specs
  "Read and decorate ugen metadata to create final UGEN-SPECS"
  {:author "Jeff Rose"}
  (:use [clojure.pprint]
        [clojure.set :only [difference]]
        [subtide.helpers lib]
        [subtide.sc.machinery.ugen defaults common special-ops categories sc-ugen])
...
(defn- specs-from-namespaces
  ...
  [namespaces]
  (reduce (fn [mem ns]
            (let [full-ns (symbol (str "subtide.sc.machinery.ugen.metadata." ns))
...
(defn- with-categories
  ...
  [spec]
  (assoc spec :categories (get UGEN-CATEGORIES (subtide-ugen-name (:name spec)) [])))
...
(defn- with-fn-names
  ...
  * (env-gen:kr ...)
  ...
  (let [rates (:rates spec)
        rate-vec (vec rates)
        base-name (subtide-ugen-name (:name spec))

...
(defn- validate-target-pos!
  ...
  (when-not (valid-synth-node-pos pos)
    (throw (ex-info (str "Invalid synth node target. Was expecting one of " valid-synth-node-pos ", found: " pos)
                    {}))))
...
(defn- extract-target-pos-args*
  ...
  (let [initial-arg (first args)]
    (when (some #{initial-arg}
      ...
      (throw (ex-info "Specifying :target and :position at the start of the standard synth args is now deprecated. Use new vec form i.e. [:after my-g] or [:tail foo-g]"
                      {})))
    (when (and (vector? initial-arg)
               (not= 2 (count initial-arg)))
      (throw (ex-info (str "Target and position vector must contain only two arguments (i.e. [:after my-g]). Found: " initial-arg)
                      {})))

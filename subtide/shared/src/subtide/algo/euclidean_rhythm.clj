(ns subtide.algo.euclidean-rhythm
...
(defn- build [level counts remainders]
  (let [pattern ((fn build [pattern level]
                   (case level
                     -1 (conj pattern 0)
                     -2 (conj pattern 1)
                     (let [pattern (reduce build pattern (repeat (nth counts level 0) (dec level)))]
                       (if-not (zero? (nth remainders level 0))
                         (build pattern (- level 2))
                         pattern))))
                 [] level)
...
(defn euclidean-rhythm
  ...
  [pulses steps]
  (when (> pulses steps)
    (throw (ex-info "the pulses cannot exceed the steps" {})))
  (loop [counts  []
         remainders [pulses]

...
(defn midi-mk-full-device-key
  "Returns a unique key for the specific device. In the case of multiple
   ...
   namespace via or a raw MIDI device map from subtide.midi. "
  [dev]
...
(defn- mk-control-key-keyword-for-agent
  [control-key]
  (midi-mk-control-key-keyword "subtide.studio.midi/update-agent-for-control-" control-key))
...
(defn- detect-midi-devices
  ...
  (->> (midi/midi-sources)
       ...
       (map #(assoc % ::dev-num (next-dev-num "subtide.studio.midi - device - " %)))
...
(defn- detect-midi-receivers
  ...
  (->> (midi/midi-sinks)
       remove-duplicate-devices
       (map #(assoc % ::dev-num (next-dev-num "subtide.studio.midi - receiver - " %)))
       (map #(assoc % ::full-device-key (midi-mk-full-device-key %)))))

(defn- add-listener-handles!
  "Adds listener handles to send incoming messages to Subtide's event
...
(defn midi-connected-devices
  "Returns a sequence of device maps for all 'connected' MIDI
   ...
   messages (such as a MIDI piano). By connected, we mean that Subtide
...
(defn midi-connected-receivers
  "Returns a sequence of device maps for all 'connected' MIDI
   ...
   receiving messages. By connected, we mean that Subtide is aware of
   the device.

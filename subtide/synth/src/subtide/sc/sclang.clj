(ns subtide.sc.sclang
  (:require
   ...
   [subtide.helpers.system :refer [get-os linux-os? mac-os? windows-os?]]
   [subtide.helpers.file :as file]
   [subtide.sc.defaults :as defaults]
   [subtide.sc.machinery.server.connection :as ov.conn]
   [subtide.sc.synth :as ov.synth])
...
(defn exec!
  ...
  [sc-clj]
  (stop-procs!)
  (let [temp-scd (io/file (str ".subtide/sc/" "_" (random-uuid) ".scd"))
...
(defn -synthdef-save!
  ...
  (if (and (io/resource resource-path)
    ...
    (let [temp-scd (io/file (str ".subtide/sc/" synthdef-name ".scd"))]
...
(defmacro defsynth
  ...
  ;; The .scd file will be stored in .subtide/sc/, should you wish to inspect it

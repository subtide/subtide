(ns subtide.sc.machinery.server.connection
  (:require
   ...
   [subtide.helpers.file :as file :refer [dir-exists? resolve-tilde-path]]
   [subtide.helpers.lib :refer [deref! print-ascii-art-subtide-logo windows-sc-path]]
   [subtide.helpers.process-info :as process-info]
   [subtide.helpers.shell :refer [shellquote]]
   [subtide.helpers.system :refer [get-os linux-os? windows-os?]]
   [subtide.libs.deps :as deps]
   [subtide.libs.event :as event]
   [subtide.osc :as osc]
   [subtide.osc.decode :refer [osc-decode-packet]]
   [subtide.sc.defaults :as defaults]
   [subtide.sc.machinery.server.args :as args]
   [subtide.sc.machinery.server.comms :refer [server-osc-peer* server-recv server-snd]]
...
(defn- subtide-ports []
  (sort (filter #(re-find #"Subtide|SuperCollider" %)
...
(defn connect-jack-ports
  ...
  ([n-channels]
   (try
     (jack/connect (take n-channels (subtide-conns)))
     (catch Exception e
...
(defn- external-connection-runner
  ...
  (let [sc-server (osc/osc-client host port false)]
    (osc/osc-listen sc-server #(event/event [:subtide :osc-msg-received] :msg %))
    (dosync
...
(defn- osc-msg-decoder
  "Decodes incoming osc message buffers and then sends them as subtide events."
  [buf]
  (event/event [:subtide :osc-msg-received] :msg (osc-decode-packet buf)))
...
(defn scsynth-path []
  (let [sc-config (config/config-get :sc-path)
    ...
    (log/info "Found SuperCollider server: " match " (" (cond
                                                          sc-config
                                                          (str "configured in " config/SUBTIDE-CONFIG-FILE)
                                                          @sc-path
                                                          "PATH"
                                                          @sc-wellknown

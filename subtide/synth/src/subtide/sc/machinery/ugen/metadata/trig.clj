(ns subtide.sc.machinery.ugen.metadata.trig
...
(def specs
  [{:name "TWindex"
   ...
   {:name "SendTrig"
    :summary "Send a /tr OSC message to Subtide"
    :args [{:name "in"
            :default 0.0
            :doc "input trigger signal"}
   ...
   {:name "SendReply"
    :summary "Send information via OSC to Subtide"
    :args [{:name "trig"
            :default 0.0
            :doc "Input trigger signal"}
    ...
    :doc "Send an array of values from the server via an
          ...
          When the trig tr triggers, Subtide will receive an event that

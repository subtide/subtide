(ns subtide.sc.node
  (:require
   ...
   [subtide.helpers.seq :refer :all]
   [subtide.libs.counters :refer :all]
   [subtide.libs.event :refer :all]
   [subtide.sc.defaults :refer :all]
   [subtide.sc.dyn-vars :refer :all]
   [subtide.sc.protocols :as protocols]
   [subtide.sc.server :refer :all]))
    ...
    (condp = inme
      ...
      (throw
       (IllegalArgumentException.
        (str "Unexpected value for subtide.sc.dyn-vars/*inactive-node-modification-error*: "
             inme
...
(defn node?
  "Returns true if obj is a synth node i.e. a SynthNode or a SynthGroup
   ...
   from :subtide.sc.node/synth-node"
  [obj]
  (isa? (type obj) ::node))
  ...
  (let [snode (get @active-synth-nodes* id)]
    ...
    (if snode
      (do
        ...
        (event [:subtide :node-destroyed (:id snode)] {:node snode}))
  ...
  (let [snode (get @active-synth-nodes* id)]
    ...
    (if snode
      (do
        ...
        (event [:subtide :node-created (:id snode)] {:node snode})
  ...
  (let [snode (get @active-synth-nodes* id)]
    ...
    (if snode
      (do
        ...
        (event [:subtide :node-paused (:id snode)] {:node snode}))
...
(defn- node-started
  ...
  (let [snode (get @active-synth-nodes* id)]
    ...
    (if snode
      (do
        (reset! (:status snode) :live)
        (event [:subtide :node-started (:id snode)] {:node snode}))
...
(defn node-map-n-controls*
  ...
  (assert (isa? (type start-bus) :subtide.sc.bus/bus) "Invalid start-bus")
  (let [node-id (to-sc-id node)]

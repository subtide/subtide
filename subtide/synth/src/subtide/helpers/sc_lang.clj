(ns subtide.helpers.sc-lang
  "Convert a subset of sclang to Subtide

  Convert strings of SuperCollider's built-in language to Subtide forms. Fairly
  ...
  snippet of sc-lang directly inside Subtide synth definitions.
...
(defn- fnames [tokens]
  (let [code (z/seq-zip tokens)
        subtide-ugen-name (comp
                             #(.replaceAll % "\\.(ar|kr)" ":$1")
                             subtide-ugen-name)]
   (z/root (zip-map #(if (string? (z/node %))
                       (z/edit % subtide-ugen-name)
                       %)
                    code))))

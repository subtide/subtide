(ns subtide.examples.midi.basic
  (:use [subtide.live]
        [subtide.synth.sts :only [prophet]]))
...
;; Subtide's event system. By default all note-making MIDI devices will
;; emit a [:midi :note-on] event. Make sure you attach a MIDI piano-like
;; device *before* booting Subtide and the following will work:
...
;; See subtide.examples.midi.keyboard for a built-in way of achieving
...
;; All attached MIDI devices are registered when Subtide booted.
;; You can look at the list of attached devices with:

(midi-connected-devices)

;; The MIDI devices are not guaranteed to be registered in the same
;; order every time you boot Subtide, so you can't rely on the index of
...
;; Once I have my device, I can find the unique key that Subtide's

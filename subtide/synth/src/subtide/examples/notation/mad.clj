...
(definst tone [note 60 amp 0.3 dur 0.4]
  (let [snd (-> note midicps sin-osc)
        env (-> (perc 0.01 dur) (env-gen :action FREE))]
    (* env snd amp)))
...
(comment
  (do
    (p (map #(assoc % :synth ks1-demo) (pattern g-minuet-left-hand 25)))
    (p (map #(assoc % :synth ks1-demo) (pattern g-minuet-right-hand 25))))
  (stop)
  )

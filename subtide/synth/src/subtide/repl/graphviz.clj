(ns subtide.repl.graphviz
  (:use [subtide.repl.debug :only [unified-sdef]]
        [subtide.sc.machinery.ugen.metadata.unaryopugen]
        [subtide.sc.machinery.ugen.metadata.binaryopugen]
        [subtide.helpers.system :only [mac-os? linux-os?]]
        [subtide.helpers.file :only [mk-tmp-dir! mk-path]]
        [clojure.java.shell]))
...
(defn graphviz
  "Generate dot notation for synth design.
   (see subtide.repl.debug/unified-sdef)"
  [s]

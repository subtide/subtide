(ns subtide.repl.ugens
  (:use [clojure.repl]
        [subtide.sc.machinery.ugen fn-gen specs]
        [subtide.helpers lib doc string]))
...
(defn- print-ugen-summaries
  ...
  (dorun
   (map
    (fn [spec]
      (let [n (str (subtide-ugen-name (:name spec)))
...
(defn pretty-ugen-doc-string
  ...
  ([ug-spec ^java.lang.String ns-str]
   (let [ns-str (if (or
     ...
     (str "-------------------------"
          "\n"
          ns-str (subtide-ugen-name (:name ug-spec))
          "\n"
          (:full-doc ug-spec)
          "\n\n"))))
...
(defmacro find-ugen
  ...
  (let [search-terms     (map #(if (symbol? %) (str %) %) search-terms)
        ...
        names            (map #(subtide-ugen-name (:name %)) specs)

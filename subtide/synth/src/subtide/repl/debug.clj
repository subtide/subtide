...
(defn- unify-input
  ...
  (if (= -1 (:src input))
    ...
    (let [idx (:src input)
          ...
          id (if (control-ugen-name? (subtide-ugen-name n))
...
(defn- fix-input-refs
  ...
  (map (fn [input]
         (if (number? input)
           input
           (let [n (subtide-ugen-name (:name input))]
             {:name n
...
(defn- find-control-offset
  ...
  (let [ctl-ugs (filter (fn [ug]
                          (control-ugen-name? (subtide-ugen-name (:name ug))))
                        (:ugens sdef))]
    (loop [result 0
           ugs ctl-ugs]
      (when (empty? ugs)
      ...
      (let [ug (first ugs)]
        (if (and (= c-name (subtide-ugen-name (:name ug)) )
...
(defmethod sdef :subtide.sc.machinery.synthdef/synthdef
  [s]
  s)

(defmethod sdef :subtide.sc.machinery.synthdef/imported-synthdef
  [s]
  s)
...
(defmethod sdef subtide.sc.synth.Synth
  [s]
  (sdef (:sdef s)))

(defmethod sdef subtide.studio.inst.Inst
  [s]
  (sdef (:sdef s)))
...
(defn unified-sdef
  ...
  order. Useful for comparing two ugen synthdefs (i.e. an Subtide and
  SCLang synthdef) side-by-side."
  (unify-sdef (sdef s)))

(defn pp-unified-sdef
  "Pretty print a unified version of an subtide or sc synth. See

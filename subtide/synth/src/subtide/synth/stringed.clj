...
;;
;; See subtide/examples/instruments/guitar_synth.clj for example usage.
;;
...
(defmacro gen-stringed-synth
  ...
  (let [note-ins (if (= num-strings 1)
        ...
        env-gen-fn (if free-on-silence
                     '(fn [x] (subtide.sc.ugens/env-gen
                               (subtide.sc.envelope/asr 0.0001 1 0.1)
                               :gate (second x)
                               :action subtide.sc.ugens/FREE))
                     '(fn [x] (subtide.sc.ugens/env-gen
                               (subtide.sc.envelope/asr 0.0001 1 0.1)
                               :gate (second x))))]
    `(defsynth ~name

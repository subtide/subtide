(ns subtide.libs.asset
  ...
  (:use [clojure.java.io :only [file]]
        ...
        [subtide.helpers file zip string]
        [subtide.config.store :only [SUBTIDE-DIRS]]
        [subtide.helpers.ns :only [immigrate]])
  (:require [subtide.libs.asset.store]))

(immigrate 'subtide.libs.asset.store)

(def ^:dynamic *cache-root* (:assets SUBTIDE-DIRS))
...
(defn- download-asset-file
  ...
  (binding [*verbose-subtide-file-helpers* 2]

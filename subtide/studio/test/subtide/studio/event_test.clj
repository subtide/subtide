(ns subtide.studio.event-test
  (:require
   [clojure.test :refer [deftest testing is are]]
   [clojure.test.check.clojure-test :refer [defspec]]
   [clojure.test.check.generators :as gen]
   [clojure.test.check.properties :refer [for-all]]
   [subtide.music.rhythm :refer [metronome metro-start]]
   [subtide.studio.event :as event]
   [subtide.studio.pattern :as pattern]
   [subtide.studio.transport :as transport]
   [subtide.sc.node]))

(def player-init-keys [:align :clock :offset :proto :quant])
(def player-run-keys [:beat :playing])

(def dummy-instrument (reify subtide.sc.node/IControllableNode

(ns subtide.sc.trig
  (:use [subtide.libs event counters]
        [subtide.sc.node]))

(on-sync-event "/tr"
               (fn [{path :path args :args}]
                 (let [[node-id trig-id val] args]
                   (event [:subtide :trigger trig-id] :val val)
                   (event [:subtide :trigger node-id trig-id] :val val)))
...
(defn on-trigger
  ...
  ([trig-id handler key]
     (add-handler on-event [:subtide :trigger trig-id] handler key))
  ([node trig-id handler key]
     (add-handler on-event [:subtide :trigger (to-sc-id node) trig-id] handler key)))
...
(defn on-latest-trigger
  ...
  ([trig-id handler key]
     (add-handler on-latest-event [:subtide :trigger trig-id] handler key))
  ([node trig-id handler key]
     (add-handler on-latest-event [:subtide :trigger (to-sc-id node) trig-id] handler key)))
...
(defn on-sync-trigger
  "Registers a standard on-sync-event handler with key which will call
   ...
   Trigger handler can be removed with remove-event-handler."
  ([trig-id handler key]
     (add-handler on-sync-event [:subtide :trigger trig-id] handler key))
  ([node trig-id handler key]
     (add-handler on-sync-event [:subtide :trigger (to-sc-id node) trig-id] handler key)))

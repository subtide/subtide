(ns subtide.sc.machinery.ugen.sc-ugen
  "Records and fns for representing SCUgens. These are to be distinguished from
  ugens which are Subtide functions which compile down into SCUGens. Trees of
...
(defmethod print-method SCUGen [ug ^java.io.Writer w]
  (.write w (str "#<sc-ugen: " (subtide-ugen-name (:name ug)) (:rate-name ug) " [" (count-ugen-args ug) "]>")))

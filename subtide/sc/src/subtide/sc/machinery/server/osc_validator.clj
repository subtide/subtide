...
(defn Integer? [val] (instance? Integer val))
(defn Float? [val] (instance? Float val))
...
(def TYPES
  {
   ...
   :ctl-handle       {:desc      "an integer or a string representing a control"
                      :validator (some-fn Integer? string?)}
...
(defn- many-type->type
  ...
  (let [n (name many-type)]
    (keyword (.substring n 0 (dec (count n))))))
...
(defn- alternating?
  [type]
  (str/starts-with? (name type) "ALTERNATING"))
...
(defn- error-args
  [sig args]
  (some (fn [[type arg :as e]]
          (when-not (valid? type arg)
            e))
...
(defn- checked-snd
  ...
  (let [s-arity (sig-arity path)
    ...
    (when (arity-mismatch? s-arity a-arity)
      (let [err-string (str "Failed attempting to send an OSC message to SuperCollider server. Reason: you didn't provide the correct number of arguments for OSC message "
                            path ". Expected " s-arity ", got " a-arity " Type sig: " sig " Arg list: " args)]
        (log/error err-string)
        (throw (ex-info err-string {}))))
